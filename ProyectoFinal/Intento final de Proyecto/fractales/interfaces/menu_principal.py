...
class SimpleFractalMenu(QMainWindow):
    ...
    def open_sierpinski(self):
        """Abre la ventana del Triángulo de Sierpinski Ultra-Fluido."""
        try:
            if self.sierpinski_window is None:
                from .sierpinski_navegable import SierpinskiNavigableWindow
                self.sierpinski_window = SierpinskiNavigableWindow()
            
            self.sierpinski_window.show()
            self.sierpinski_window.raise_()
            self.sierpinski_window.activateWindow()
        except Exception as e:
            print(f"Error abriendo Sierpinski Ultra-Fluido: {e}")
